...
public class SearchOffersValidator : AbstractValidator<SearchOffersQuery>
{
	...
	{
		RuleFor(s=>s.Condition)
			.Must(x => Condition.SupportedConditions.Contains(x))
			.When(s => s.Condition != null)
			.WithMessage("Condition must be one of the following: New, Used, Good");
		...
		RuleFor(s => s.CreatedAscending)
			.Must(x => x is null or false).When(s => s.PriceAscending is true).WithMessage("Can not sort by both price and created date");


		RuleFor(s => s.PriceAscending)
			.Must(x => x is null or false).When(s => s.CreatedAscending is true).WithMessage("Can not sort by both price and created date");
